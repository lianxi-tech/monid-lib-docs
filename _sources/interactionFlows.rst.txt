...
This section offers an overview of the interaction flows supported by the MONiD Library.
...
.. note:: The following sections assume you have already created an identity. If you have not yet created an identity, check out the `Getting Started <https://monid-lib.readthedocs.io/en/latest/gettingStarted.html>`_ section.
...
.. code-block:: typescript
  ...
  import {constraintFunctions} from '@monid/lib/js/interactionTokens/credentialRequest'
...
.. note:: For further documentation and examples explaining how to create and send
 ...
 the the `integration tests <https://github.com/lianxi-tech/monid-lib/tree/master/tests/integration>`_, and the `MONiD SDK documentation <https://github.com/lianxi-tech/monid-sdk/blob/main/docs/guides/interaction_flows.md>`_.
...
.. note:: The interaction messages can be sent to other agents for processing via any preferred channel. In order to allow `SmartWallet <https://github.com/lianxi-tech/monid-smartwallet-app>`_ users to consume the message, it can be encoded as a QR code, or via "Deep Links"..
...
.. code-block:: typescript
  ...
  const credentialRequest = MONiDLib.parse.interactionToken.fromJWT(enc)
...
.. code-block:: typescript
  ...
  const credentialResponse = await MONiDLib.parse.interactionToken.fromJWT(receivedJWTEncodedResponse)
...
The MONiD Library also allows for the issuance  of verifiable credentials. Similarly to the flow
...
.. code-block:: typescript
  ...
  import { CredentialRenderTypes } from '@monid/lib/interactionTokens/interactionTokens.types'
...
by the MONiD Smartwallet. The currently supported options are:
...
.. code-block:: typescript
  ...
  const credentialOffer = MONiD.parse.interactionToken.fromJWT(enc)
  identityWallet.validateJWT(credentialRequest)
